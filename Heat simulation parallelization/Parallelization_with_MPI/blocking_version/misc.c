...
int initialize( algoparam_t *param)
{
	...
	// number of X and Y points (including border)
    ...
    {
	/* top row */
	if((param->startIndexY) == 0){
		for( j=0; j< npX; j++ )
		{
			dist = sqrt( pow((double)(j + param->startIndexX)/(double)(np-1) -
	...
	/* bottom row */
	if(param->endIndexY == param->act_res){
		for( j=0; j < npX; j++ )
		{
			dist = sqrt( pow((double)(j + param->startIndexX)/(double)(np-1) -
					param->heatsrcs[i].posx, 2)+
	...
	/* leftmost column */
	if(param->startIndexX == 0){
		for( j=1; j<npY-1; j++ )
		{
			dist = sqrt( pow(param->heatsrcs[i].posx, 2)+
				pow((double)(j + param->startIndexY)/(double)(np-1) -
	...
	/* rightmost column */
	if(param->endIndexX == param->act_res){
		for( j=1; j<npY-1; j++ )
		{
	    	...
	    	double *unew, unsigned newx, unsigned newy )
{
    ...
    for( i=0; i<stopy; i++ ){
       ii=stepy*i - startIndexY;
       for( j=0; j<stopx; j++ ){
          jj=stepx*j - startIndexX;
          temp = 0;
          for ( k=0; k<stepy; k++ ){
	       	for ( l=0; l<stepx; l++ ){
	       		if (0<=ii+k && ii+k<npY && 0<=jj+l && jj+l<npX)
		           temp += uold[(ii+k)*npX+(jj+l)] ;
	        }
	      }
